...
use super::{
    dap::protocol::{
        self as dap_protocol, BreakpointEventBody, ContinueArguments, ContinuedEventBody, DAPCodec,
        DecoderResult, Either, EventBody, InitializeRequestArguments, NextArguments,
...
use crate::{
    ...
    util::{get_debuggers_dir, get_os_and_cpu_architecture, log_err, log_ret_err, ret_err},
};
...
pub struct Debugger {
    ...
    pending_outgoing_requests: Arc<Mutex<HashMap<String, oneshot::Sender<ResponseResult>>>>,
...
impl DebuggerAPI for Debugger {
    ...
    async fn setup(
        ...
        _existing_debugger_port: Option<String>,
    ) -> Result<()> {
        log_ret_err!(
...
impl Debugger {
    ...
    async fn launch(&mut self) -> Result<()> {
        let msg = format!(
            ...
            self.command, "dotnet debugger", self.command_args,
        );
        self.log_msg(VadreLogLevel::DEBUG, &msg).await?;

        self.download_plugin().await?;

        let path = self.get_debugger_binary_path()?;

        if !path.exists() {
            bail!("The binary doesn't exist: {}", path.to_str().unwrap());
        }

        let args = vec!["--interpreter=vscode".to_string()];
                ...
                tokio::select! {
                    ...
                    Some((message_type, sender)) = debugger_sender_rx.recv() => {
                        ...
                        let message = ProtocolMessage {
                            seq: Either::Second(seq.to_string()),
                            type_: message_type
                        };
                        match sender {
                            Some(sender) => { pending_outgoing_requests.lock().await.insert(seq.to_string(), sender); },
                            None => {},
                        };
                        framed_stream
                            .send(message)
    ...
    async fn init_process(
        ...
        dap_shared::do_send_request_and_await_response(
            RequestArguments::initialize(InitializeRequestArguments::new("netcoredbg".to_string())),
            self.debugger_sender_tx.clone(),
        )
        .await?;
        ...
        dap_shared::do_send_request(
            RequestArguments::launch(Either::Second(serde_json::json!({
    ...
    fn get_debugger_binary_path(&self) -> Result<PathBuf> {
        let binary_name = format!("netcoredbg{}", EXE_SUFFIX);
    ...
    async fn download_plugin(&self) -> Result<()> {
        ...
        if !path.exists() {
            self.log_msg(VadreLogLevel::INFO, &format!("So downloading it"))
                .await?;
        ...
        match args {
            RequestArguments::runInTerminal(args) => {
                ...
                let response = ProtocolMessageType::Response(Response {
                    request_seq: Either::Second(request_id),
                    success: true,
    ...
    async fn handle_response(
        ...
        let request_id = response.request_seq.second();
    ...
    async fn handle_event_breakpoint(
        ...
        let is_resolved = Debugger::breakpoint_is_resolved(&breakpoint_event.breakpoint);
    ...
    fn breakpoint_is_resolved(breakpoint: &dap_protocol::Breakpoint) -> bool {
        tracing::info!("Breakpoint: {:#?}", breakpoint);
        false
    }
...
impl Debug for Debugger {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NetCoreDbgDebugger")
            .field("id", &self.id)
